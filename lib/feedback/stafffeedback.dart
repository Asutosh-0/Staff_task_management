...
class _TeacherFeedbackState extends State<TeacherFeedbackpage> {
  ...
  Map<String, List<Map<String, dynamic>>> filterData(
    ...
    if (course == 'All') {
      ...
      data.forEach((week, items) {
        final filteredItems =
        items.where((item) => item['teacher_id'] == teacherId).toList();
    ...
    data.forEach((week, items) {
      final filteredItems = items
          .where((item) => item['cource'] == course && item['teacher_id'] == teacherId)
          .toList();
      if (filteredItems.isNotEmpty) {
        filteredData[week] = filteredItems;
      }
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.info_outline, size: 60, color: Colors.grey),
                        SizedBox(height: 16),
                        Text('No data available for the selected course',
                            style: TextStyle(
                                fontSize: 18,
                                color: Colors.grey[700],
                                fontWeight: FontWeight.w500)),
                          ...
                          return Padding(
                            ...
                            child: ListTile(
                                ...
                                trailing: Column(
                                    ...
                                    children: [
                                      Text(
                                        'Feedback: ${item['average_score']}', // String interpolation
                                        style: TextStyle(
                                          fontSize: 10,
                                          fontWeight: FontWeight.w500,
                                          color: Colors.black,
                                      ...
                                      Text(
                                        '${item['course']}', // Corrected: Just the course name
                                        style: TextStyle(
                                          fontSize: 10,
                                          color: Colors.black,
                                        ),
