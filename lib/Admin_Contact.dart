...
class _ContactPage extends State<Admin_ContactPrev> {
  ...
  @override
  Widget build(BuildContext context) {
    const _color1 = Color.fromARGB(255, 7, 7, 7);
            ...
            child: Column(
              children: [
                for (String course in allowedCourses)
                  CardItem(course, const Color(0xFF090808)),
              ],
            ),
          ),
        ),
...
class _Contact extends State<Contact> {
  ...
  Widget _buildYearButton(String label, String year) {
    Color customColor = Colors.black; // Black background
    Color innerColor = Colors.white; // White inner elements
    Color selectedColor = Colors.black; // Light grey for selected elements
    Color textColor = Colors.black; // Black text
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          '${widget.course}',
          style: const TextStyle(fontWeight: FontWeight.bold ,color: Colors.white,fontFamily: 'Times New Roman',),
